...
PROJECT_NAME=policy-mix-code
OUTPUT_ROOT=${WORKING_DIR}/outputs/${PROJECT_NAME}

TRAIN_FILE=./data/livecodebench_v6/train.parquet
VAL_FILE=./data/livecodebench_v6/test.parquet
...
submit_job() {
  ...
  local k_off="$5"
  local rollout_n="$6"
  local method_tag="${7:-grpo}"
  local use_entropy_aware_mixing="${8:-true}"
  local use_rollout_correction="${9:-true}"
  local filter_groups_enable="${10:-false}"
  ...
  if [ "${algo}" = "mixed_policy" ]; then
    if [ "${use_entropy_aware_mixing}" = "true" ]; then
      run_name="code_${method_tag}__${STUDENT_MODEL_TAG}--${TEACHER_MODEL_TAG}__bs${TRAIN_BATCH_SIZE}__${k_on}on${k_off}off__rc-${use_rollout_correction}${filter_tag}__EASD-topk${ENTROPY_TOP_K}_${ENTROPY_AWARE_MIXING}_${ENTROPY_AWARE_ALPHA}__seed${seed}"
    else
      run_name="code_${method_tag}__${STUDENT_MODEL_TAG}--${TEACHER_MODEL_TAG}__bs${TRAIN_BATCH_SIZE}__${k_on}on${k_off}off__rc-${use_rollout_correction}${filter_tag}__SD__seed${seed}"
    fi
  else
    run_name="code_${algo}__${STUDENT_MODEL_TAG}__bs${TRAIN_BATCH_SIZE}__n${rollout_n}${filter_tag}__seed${seed}"
  fi
  ...
  sbatch_output="$(
    sbatch \
      ...
      "${WORKING_DIR}/experiments/code/run_single_job.sh"
  )"
...
for rep in $(seq 1 "${REPEATS}"); do
  ...
  for split in "${MIXED_SPLIT_PAIRS[@]}"; do
    ...
    for method in "${MIXED_METHODS[@]}"; do
      method_tag="${method%%:*}"
      use_entropy_aware_mixing="${method##*:}"
      for filter_groups_enable in "${FILTER_GROUPS_ENABLE_OPTIONS[@]}"; do
        for use_rollout_correction in "${USE_ROLLOUT_CORRECTION_OPTIONS[@]}"; do
          submit_job "mixed_policy" "${rep}" "${seed}" "${k_on}" "${k_off}" "${rollout_n}" "${method_tag}" "${use_entropy_aware_mixing}" "${use_rollout_correction}" "${filter_groups_enable}"
  ...
  for rollout_n in "${GRPO_ROLLOUT_NS[@]}"; do
    for filter_groups_enable in "${FILTER_GROUPS_ENABLE_OPTIONS[@]}"; do
      submit_job "grpo" "${rep}" "${seed}" 0 0 "${rollout_n}" "grpo" "true" "true" "${filter_groups_enable}"
      submitted=$((submitted + 1))
    done
  done
done
