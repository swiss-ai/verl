...
ROLLOUT_N=8
ENABLE_FILTER_GROUPS=true # DAPO-like filtering
SEED=42
...
resolve_run_name() {
  ...
  if [ -z "${RUN_NAME}" ]; then
    RUN_NAME="${PROJECT_NAME}__${model_tag}__seed${SEED}__${date_time}"
  fi
...
build_overrides() {
  overrides=(
    ...
    # "trainer.save_freq=10"  Try test only, no value for save freq (should be -1 by default)
    "+rollout.test_freq=2"
