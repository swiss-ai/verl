...
| `async_training.retain_stale_kv_cache`        | SGLang-only opt-in to retain stale prefix/KV cache across in-flight partial-rollout updates     |
...
* `async_training.retain_stale_kv_cache`

  SGLang-only experimental option. When enabled with async partial rollout, parameter synchronization cancels and
  resumes partial generations without flushing SGLang prefix/KV cache, allowing resumed `prompt + partial_response`
  requests to reuse KV produced by an older policy. This may increase rollout GPU memory pressure. Enable SGLang
  Prometheus metrics and monitor `sglang:cache_hit_rate` and `sglang:cached_tokens_total` as a cross-check.
