...
class RayVerticalGenTrainer(RayPPOTrainer):
    """PPO Trainer with dynamic rollout generation.
    
    This trainer extends RayPPOTrainer to support adaptive rollout generation where:
    - Each problem receives a small number of rollouts per batch (e.g., 2)
    - Problems continue receiving rollouts until meeting criteria (k correct or budget exhausted)
    - Training batches are accumulated from completed problems
    - Gradient updates occur when training batch is full
    
    This approach is more sample-efficient than fixed rollout counts.
    """
            ...
            while self.train_dataloader.sampler.has_next_batch():
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("split_rollouts", timing_raw, color="magenta"):
                        ...
                        # Split into continue_generation vs ready_for_training
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        # Rewards are already computed above for dynamic rollout splitting
                        # They are in batch.batch["token_level_scores"] 
                ...
                metrics.update(
                    {
                        "training/global_step": self.global_steps,
                        "training/intermediate_steps": intermediate_steps,
                        "training/total_step": total_steps,
                        "training/epoch": epoch,
                    }
                )
