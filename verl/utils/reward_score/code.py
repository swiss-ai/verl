...
def run_test_func(completion, test_input, test_output, fn_name, namespace=None):
    ...
    try:
        if isinstance(test_input, dict):
            ...
            result_output = namespace[func_name](*copy.deepcopy(test_input))
        else:
            try:
                parsed_input = json.loads(test_input)
                if isinstance(parsed_input, dict):
                    result_output = namespace[func_name](**parsed_input)
        ...
        try:
            ...
            if isinstance(test_output, str):
                try:
                    expected_output = json.loads(test_output)
                except Exception:
                    expected_output = test_output
            else:
                expected_output = test_output
            rhs = _to_safe_jsonable(expected_output)
...
def run_tests_for_one_example(test_cases, completion, send_conn, sparse_rewards, test_idx):
    ...
    test_type = test_cases.get("testtype", "functional" if test_cases.get("fn_name") else "stdin")
...
def run_tests(test_cases: dict, solution, sparse_rewards, max_test_cases):
    ...
    timeout = test_cases.get("time_limit", DEFAULT_TIMEOUT)
    timeout_per_test_case = float(timeout) if timeout is not None else DEFAULT_TIMEOUT
...
def compute_score(solution: str, ground_truth: str, extra_info=None, sparse_rewards=False, max_test_cases=None):
    ...
    timed_out = float(np.mean([1.0 if (not r["passed"]) and (r["actual"] == TIMEOUT) else 0.0 for r in records]))
    num_failed_tests = sum(1 for r in records if not r["passed"])
